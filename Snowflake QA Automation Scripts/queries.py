...
def run_query(mapping_login, query):
    ...
    try:
        ...
        con = snowflake.connector.connect(
                user = os.getlogin() + '@initiative.com',
                account = mapping_login['account'],
                role = mapping_login['role'],
                warehouse = mapping_login['warehouse'],
                database = mapping_login['database'],
...
def select_table_by_query(mapping_login, mapping_select, days = 90, date_start = None, date_end = None):
    ...
    date_column = mapping_select['variable_date']
    ...
    variable_str = ', '.join(mapping_select['variable'])
    ...
    query = '''
    SELECT CAST({date_str} AS VARCHAR) AS {date_str}, 
        {variable_str}, 
        {value_str}
    ...
    '''.format(
        date_str = mapping_select['variable_date'],
    ...
    df[date_column] = df[date_column].map(lambda x: datetime.date(datetime.strptime(x, '%Y-%m-%d')))
